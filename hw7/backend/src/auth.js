...
const registerUser =(req, res) => {
	 Profile.find({'username' : req.body.username}, function(err, foundUser) {
	 	if(!foundUser.length){
		     const salt =  Math.random().toString(36).substring(7);
		     const newAuth = new Auth({"username" : req.body.username, "salt":salt, "hash":md5(req.body.password+salt)})
		     const followerList = new Follower({"username":req.body.username, "followers":[]})
		     const newUser = new Profile({"username" : req.body.username,
			...
			followerList.save()
		     	newUser.save(function(err, user){
		     		!err?res.send({result : 'success', username : user.username}) : res.send("Failed")
		     	})
		     })
	 	}
	 	else {
...
const loginUser =(req, res) => {
	...
	Auth.find({username : req.body.username}, (err, user) => {
		if(user.length){
			if(user[0].hash === md5(req.body.password + user[0].salt))
			{
				...
				res.cookie(cookieKey,sessionId,{maxAge:3600*1000, httpOnly : false});
...
const updatepassword = (req,res) => {
	...
	Auth.find({'username':req.username},function(err, user){
			const salt =  Math.random().toString(36).substring(7);
			Auth.update({'username': req.username},{$set : {'salt' : salt, 'hash':md5(req.body.password+salt)}})
			.then(res.send({ username: req.username, status: 'success' }))
	})
}
...
const logout= (req, res) => {
	//Removes Key from REDIS and deletes the browser cookie
		...
		res.cookie(cookieKey,"",{expires:new Date()})
		res.sendStatus(200)
}
module.exports = app => {
