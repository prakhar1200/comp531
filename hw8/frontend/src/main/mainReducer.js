...
const MainReducer = (state = initialState , action) => {
		...
		case 'INITIAL_STATE':
		const currentArticles = action.payload[0].articles.map((article) => {return {...article, displayComment : false}})
		return Object.assign({}, state, {articles : currentArticles, backupArticle :currentArticles, 
		 userHeadline : action.payload[1].headlines[0].headline, userAvatar:action.payload[2].avatars[0].avatar})  
		...
		case 'POST_ARTICLE':
		return Object.assign({}, state, {articles :[{...action.payload.articles[0],displayComment : false}, ...state["articles"]]})
		...
		return Object.assign({}, state, {articles : state.backupArticle})
